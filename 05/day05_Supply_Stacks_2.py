...
def rearrangement_procedure_9001(list_of_instructions, stack_lists):
    for index, move in enumerate(list_of_instructions):

        current_instructions = move.split(" ")
        crate_count = int(current_instructions[1])
        origin_stack = int(current_instructions[3]) - 1
        destination_stack = int(current_instructions[5]) - 1

        lifted_crates = stack_lists[origin_stack][-crate_count:]
        stack_lists[origin_stack] = stack_lists[origin_stack][:-crate_count]
        stack_lists[destination_stack].extend(lifted_crates[:])
...
if __name__ == '__main__':
    ...
    final_stack_lists = rearrangement_procedure_9001(instructions, initial_stack_lists)
